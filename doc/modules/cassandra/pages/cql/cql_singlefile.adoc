...
The `id.field = <term>` form of `<assignment>` is for setting the value
...
UPDATE users SET favs = favs + \{ `movie' : `Casablanca' } WHERE id =
`jsmith'
...
introduction of link:#constants[blobs constants] to allow inputting
blobs. Do note that while inputting blobs as strings constant is still
