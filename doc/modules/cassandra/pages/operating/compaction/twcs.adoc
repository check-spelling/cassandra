...
the data will be commingled in the memtables and flushed into the same
SSTable, where it will remain commingled.
* If the user's read requests for old data cause read repairs that pull
old data into the current memtable, that data will be commingled and
flushed into the same SSTable.

While TWCS tries to minimize the impact of commingled data, users should
...
such a way that it does not become commingled).
