...
`--excluded-keyspaces`::
  ...
  set the value from cassandra.yaml will be used. Please remember that
  [.title-ref]#system#, [.title-ref]#system_schema# and
...
`-f,--follow`::
  Upon reaching the end of the log continue indefinitely;;
...
`BinAuditLogger` can be further configured using its advanced options in `cassandra.yaml`.
