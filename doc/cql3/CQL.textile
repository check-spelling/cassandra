...
p. A @<variable>@ can be either anonymous (a question mark (@?@)) or named (an identifier preceded by @:@). Both declare a bind variables for "prepared statements":#preparedStatement. The only difference between an anonymous and a named variable is that a named one will be easier to refer to (how exactly depends on the client driver used).
...
@DROP ROLE@ requires the client to have @DROP@ "permission":#permissions on the role in question. In addition, client may not @DROP@ the role with which it identified at login. Finally, only a client with @SUPERUSER@ status may @DROP@ another @SUPERUSER@ role.
...
Enumerate all roles granted to @alice@, including those transitively acquired.
...
Prior to the introduction of roles in Cassandra 2.2, authentication and authorization were based around the concept of a @USER@. For backward compatibility, the legacy syntax has been preserved with @USER@ centric statements becoming synonyms for the @ROLE@ based equivalents.
...
|@inet@     |   strings            |An IP address. It can be either 4 bytes long (IPv4) or 16 bytes long (IPv6). There is no @inet@ constant, IP address should be inputted as strings|
...
UPDATE users SET favs = favs +  { 'movie' : 'Casablanca' } WHERE id = 'jsmith'
...
h4(#currentDateTime). Retrieving the current date/time
...
By default (or if @DEFAULT NULL@ is explicitly used), a column omitted from the @JSON@ map will be set to @NULL@, meaning that any preexisting value for that column will be removed (resulting in a tombstone being created). Alternatively, if the @DEFAULT UNSET@ directive is used after the value, omitted column values will be left unset, meaning that preexisting values for those column will be preserved.
...
* "@ALTER TABLE@":#alterTableStmt @ADD@ and @DROP@ now allow multiple columns to be added/removed
...
* It is now possible to group clustering columns in a relation, see "SELECT WHERE":#selectWhere clauses.
...
* @NaN@ and @Infinity@ has been added as valid float constants. They are now reserved keywords. In the unlikely case you we using them as a column identifier (or keyspace/table one), you will now need to double quote them (see "quote identifiers":#identifiers ).
